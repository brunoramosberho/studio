...
export function FriendBiometrics({ classId }: { classId: string }) {
  const { data: friends = [] } = useQuery<FriendBiometric[]>({
      ...
      <div className="space-y-1.5">
        {friends.map((f) => (
          <Link
            ...
            className="flex items-center gap-2.5 rounded-xl bg-white border border-border/40 px-3 py-2.5 shadow-sm"
          >
            <UserAvatar
