...
export function ShopPackages() {
  ...
  if (active.length === 0) {
    return (
      <div className="rounded-2xl border border-dashed border-border/60 py-16 text-center text-muted">
        No hay paquetes disponibles todavía.
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {sheetOpen && selectedPkg && isSub(selectedPkg.type) ? (
          <SubscribeSheet open={sheetOpen} onClose={() => setSheetOpen(false)} pkg={selectedPkg} />
        ) : sheetOpen && selectedPkg ? (
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          <PurchaseSheet open={sheetOpen} onClose={() => setSheetOpen(false)} pkg={selectedPkg as any} />
        ) : null}
      </AnimatePresence>
    </>
  );
