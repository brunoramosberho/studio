...
/**
 * Official-style "Add to Apple Wallet" button. A plain anchor (full navigation)
 * so iOS Safari triggers the Wallet "add pass" sheet from the `.pkpass`
 * download. Only render this for Apple platforms + active members.
 */
export function AddToAppleWalletButton({
  ...
  return (
    <a
      ...
      className={`inline-flex h-11 select-none items-center justify-center gap-2 rounded-lg bg-black px-5 text-white shadow-sm transition-transform active:scale-[0.98] ${className}`}
    >
      ...
      <span className="text-[15px] font-semibold tracking-[-0.01em]">{t("addToAppleWallet")}</span>
    </a>
  );
}
