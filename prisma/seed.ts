...
async function main() {
  ...
  const clientWithPack10 = await prisma.user.create({
    data: {
      ...
      // Birthdays anchor at UTC midnight, matching the app's write paths —
      // a local-midnight Date shifts a day when read back via UTC parts.
      birthday: new Date(Date.UTC(1995, 6, 15)),
  ...
  const clientExpired = await prisma.user.create({
    data: {
      ...
      birthday: new Date(Date.UTC(1998, today.getMonth(), today.getDate() + 2)),
  ...
  const clientUnlimited = await prisma.user.create({
    data: {
      ...
      birthday: new Date(Date.UTC(1992, 11, 3)),
  ...
  const clientPrimeraVez = await prisma.user.create({
    data: {
      ...
      name: "Lucía Martínez",
      role: Role.CLIENT,
      birthday: new Date(Date.UTC(2000, 3, 22)),
  ...
  const clientNoPackage = await prisma.user.create({
    data: {
      ...
      name: "Fernanda Ruiz",
      role: Role.CLIENT,
      birthday: new Date(Date.UTC(1997, 8, 10)),
